        ...
        const test = defineTest(
          ...
          ({ path }) => {
            it('should update the state optimistically and sequence the URL updates', async ({
              ...
              const baseSearch = getUrl({ hook, shallow, history })
              async function assertSearch(shouldEqual: string) {
                await expect(page).toHaveURL(
                  url => url.search === baseSearch + '&' + shouldEqual
                )
              }
              ...
              await assertSearch('a=1')
              await assertSearch('a=1&b=1')
              await assertSearch('a=1&b=1&c=1')
              await page.locator('#same-tick').click()
              await expect(page.locator('#client-state')).toHaveText('2,2,2')
              await assertSearch('a=2&b=1&c=1')
              await assertSearch('a=2&b=2&c=1')
              await assertSearch('a=2&b=2&c=2')
              await page.locator('#same-tick').click()
              await expect(page.locator('#client-state')).toHaveText('3,3,3')
              await assertSearch('a=3&b=2&c=2')
              ...
              await assertSearch('a=4&b=2&c=2')
              await assertSearch('a=4&b=4&c=2')
              await assertSearch('a=4&b=4&c=4')
            })
