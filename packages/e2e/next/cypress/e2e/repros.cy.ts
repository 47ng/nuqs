...
describe('Reproduction for issue #630', () => {
  ...
  function runTest(sectionToTry: string) {
    ...
    cy.contains('#hydration-marker', 'hydrated').should('be.hidden')
    cy.get('[data-testid="1-pre"]').should('have.text', '{"a":null,"b":null}')
    cy.get('[data-testid="2-pre"]').should('have.text', '{"a":null,"b":null}')
    cy.get('[data-testid="3-pre"]').should('have.text', '{"a":null,"b":null}')
    cy.get('[data-testid="4-pre"]').should('have.text', '{"a":null,"b":null}')
    cy.get(`[data-testid="${sectionToTry}-set"]`).click()
    cy.get('[data-testid="1-pre"]').should('have.text', '{"a":"1","b":"2"}')
    cy.get('[data-testid="2-pre"]').should('have.text', '{"a":"1","b":"2"}')
    cy.get('[data-testid="3-pre"]').should('have.text', '{"a":"1","b":"2"}')
    cy.get('[data-testid="4-pre"]').should('have.text', '{"a":"1","b":"2"}')
    cy.get(`[data-testid="${sectionToTry}-clear"]`).click()
    cy.get('[data-testid="1-pre"]').should('have.text', '{"a":null,"b":null}')
    cy.get('[data-testid="2-pre"]').should('have.text', '{"a":null,"b":null}')
    cy.get('[data-testid="3-pre"]').should('have.text', '{"a":null,"b":null}')
    cy.get('[data-testid="4-pre"]').should('have.text', '{"a":null,"b":null}')
