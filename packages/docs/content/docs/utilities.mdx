...
Just like [`<NuqsAdapter>{:ts}`](./options#processing-urlsearchparams), you can
specify a `processUrlSearchParams{:ts}` function to modify the search params before they are serialized.

### Alphabetical Sort

Useful for consistent canonical URLs.
