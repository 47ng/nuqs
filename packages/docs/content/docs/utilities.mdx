...
Just like [`useQueryStates{:ts}`](/docs/batching#shorter-search-params-keys), you can
...
Just like in the [`<NuqsAdapter>{:ts}`](/docs/options#processing-urlsearchparams),
...
For an object describing parsers (that you'd pass to [`createSearchParamsCache`](/docs/server-side)
or to [`useQueryStates`](/docs/batching#usequerystates)), `inferParserType` will
