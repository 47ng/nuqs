...
import { AnchoredDetails } from '@/src/components/anchored-details'
import { AnchoredTabs } from '@/src/components/anchored-tabs'
...
<HumanContent>
<AnchoredTabs items={['Vitest v1', 'Vitest v2']}>
<Tab value="Vitest v1" id="vitest-v1">
...
<Tab value="Vitest v2" id="vitest-v2">
...
<HumanContent>
<AnchoredDetails id="advanced-options">
...
- `resetUrlUpdateQueueOnMount{:ts}`: clear the shared URL update queue once when
this adapter mounts. This is `true{:ts}` by default to isolate tests. Set it to
`false{:ts}` only when a test intentionally preserves queued updates created
...
<LLMContent>
### Internal/advanced options [#advanced-options]

- `rateLimitFactor`: By default, rate limiting is disabled when testing, as it can lead to unexpected behaviours. Setting this to `1` enables rate limiting with the same factor as in production.
- `resetUrlUpdateQueueOnMount`: clears the shared URL update queue once when this adapter mounts. This is `true` by default to isolate tests. Set it to `false` only when a test intentionally preserves queued updates created before mounting the adapter.
</LLMContent>
