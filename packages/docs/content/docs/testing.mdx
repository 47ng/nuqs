...
- `resetUrlUpdateQueueOnMount`: clears the URL update queue before running the test. This is `true` by default to isolate tests, but you can set it to `false` to keep the URL update queue between renders and match production behaviour more closely.
</LLMContent>
