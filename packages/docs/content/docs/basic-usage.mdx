...
<LLMContent>
Interactive demo omitted. The code sample above shows the same `useQueryState` flow with an input, a clear button, and a rendered greeting bound to the `name` query parameter.
</LLMContent>
