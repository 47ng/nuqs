...
To move that state into the URL, pass your query state to `useReactTable` and
...
import { useReactTable, type ColumnFiltersState, type Updater } from '@tanstack/react-table'
...
Then pass the query state to `useReactTable` and update it in `onSortingChange`:
