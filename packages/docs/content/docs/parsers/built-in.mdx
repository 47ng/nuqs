...
`nuqs` provides built-in parsers for the most common types, and allows you to [define your own](/docs/parsers/making-your-own). Community parsers like [UUID](/registry/parser-uuid) are available from the [shadcn registry](/registry).

<LLMContent>
Interactive parser demos are omitted from the LLM output on this page when the surrounding code samples already describe the parser behavior.
</LLMContent>
...
<LLMContent>
Example URL: `/products?tag=books&tag=tech&tag=design`
</LLMContent>
