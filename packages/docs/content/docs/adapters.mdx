...
<Callout type="warn" title="Deprecation notice">
  ...
  Remix has converged into React Router. Migrate to React Router v7 or v8 and use
  the [React Router v7 adapter](#react-router-v7) below.
...
<Callout type="warn" title="Deprecation notice">
  ...
  Upgrade to React Router v7 or v8 and use the
  [React Router v7 adapter](#react-router-v7) below.
...
<Callout title="React Router v8">

  This adapter also works with React Router v8. The hooks it imports from
  `react-router` are unchanged in v8, so it runs as-is on nuqs@2.x.x.

  You can pin your import to `nuqs/adapters/react-router/v8`, which currently
  re-exports this v7 adapter. Pinning means a potential future divergence of
  implementation between v7 and v8 won't require changing your import.

  This support is _unofficial_ for now. Dedicated v8 tests and official support
  will land in nuqs@3.0.0.
