...
### BrowserRouter [#react-router-v6-browser]
...
### HashRouter [#react-router-v6-hash]

For applications using `createHashRouter` or `<HashRouter>`:
...
HashRouter stores the pathname and search params in the URL hash fragment
(e.g., `http://example.com/#/page?foo=bar`). This is useful for hosting
on static file servers that don't support URL rewriting.

Document fragment identifiers (e.g., `#section`) are not supported when
using HashRouter since the hash is used for routing. `MemoryRouter`
is not supported.
