...
- <SinceVersion v="2.11.0" disclaimer="inline"><span className='inline mr-1.5 not-prose' aria-hidden>⛩️</span> [Waku](#waku)</SinceVersion>
</HumanContent>
...
## <HumanContent><span className='inline mr-2 not-prose' aria-hidden>⛩️</span></HumanContent>Waku [#waku]
...
server. With `shallow: false{:ts}`, the Waku router refetches the page from
the server, so dynamic pages see the new search params in their props.
