...
<Callout title="React Router support">
  ...
  React Router v6 and Remix don't expose navigation completion, so `isLoading{:ts}`
  only covers the URL update there: use [`useNavigation{:ts}`](https://reactrouter.com/6.30.1/hooks/use-navigation)
