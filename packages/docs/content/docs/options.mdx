...
```tsx
import { useOptimisticSearchParams } from 'nuqs/adapters/remix' // or '…/react-router/v{6,7,8}'
...
<Callout title="React Router support">
  In React Router v7+ data routers, `navigate(){:ts}` returns a Promise, so
  `isLoading{:ts}` stays true until route loaders have settled (this requires
  React 19, where transitions can await promises).

  In declarative routers, React Router v6, and Remix, `navigate(){:ts}` doesn't
  report navigation completion, so `isLoading{:ts}` only covers the URL update.
  In data router apps, use
  [`useNavigation{:ts}`](https://reactrouter.com/api/hooks/useNavigation) to
  track loader states instead.
</Callout>
...
<Callout title="Example of defaults changing">
  In `nuqs@1.x.x`, `clearOnDefault{:ts}` was `false{:ts}` by default.
  <HumanContent><br/></HumanContent>
...
<LLMContent>
Interactive demo omitted. `processUrlSearchParams` can sort the merged `URLSearchParams` before the adapter writes them to the URL.
</LLMContent>
...
<LLMContent>
Interactive demo omitted. `processUrlSearchParams` can also add derived values such as a timestamp before the URL update is committed.
</LLMContent>
...
<FeatureSupportMatrix introducedInVersion='2.11.0' />
...
Some tools truncate or mis-parse those URLs. Slack, for example, cuts a link
short when a [`parseAsJson{:ts}`](/docs/parsers/built-in#json) value contains
braces. Opt out of pretty encoding to render with `URLSearchParams{:ts}`
(`application/x-www-form-urlencoded`) instead.

Call `configure{:ts}` once in **every runtime that writes URLs**. Hook updates
run in the client bundle, and `createSerializer{:ts}` often runs from
`nuqs/server{:ts}` — those are separate copies of the library, so each one
needs the call. Loaders only parse search params; they do not render the
query string, so they do not need `configure{:ts}`:
...
Pretty encoding stays the default, so existing URLs do not change unless you
opt out. The platform decodes search params before nuqs parses them, so
bookmarks written with either encoding still load.
