...
Called after the URLSearchParams have been merged via the `processUrlSearchParams{:ts}`
[adapter](/docs/adapters) prop.

Useful for modifying the search parameters before they are used by the adapter.
...
```tsx
// [!code word:processUrlSearchParamsAlphabeticalSort]
<NuqsAdapter
...
```tsx
// [!code word:processUrlSearchParamsTimestamp]
<NuqsAdapter
  processUrlSearchParams={(search) => {
