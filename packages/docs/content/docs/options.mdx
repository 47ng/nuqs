...
[Batched updates](/docs/batching#multiple-updates-batching) share one URL write and
one history entry.

In React Router based frameworks, each separate URL write with `history: 'push'{:ts}`
adds an entry, even while a loader waits. For example, a deep `q=tea` push followed
by a shallow `panel=open` push adds two entries. Back removes `panel` and keeps
`q=tea`. The shallow push does not run a loader. Loader data still comes from the
