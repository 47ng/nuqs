...
trivially serialise using `.toString(){:ts}` will need a _serialisation_ step that matches
...
for giving a beautiful, compact representation in the URL to your complex data types
<small>(because URL-encoded JSON is kinda ugly)</small>.

**Compact** here is also an important property: URLs have a size limit, much like local storage
or cookies.
...
Call updates too quickly and it will throw an error, potentially crashing your app
in the process.
...
access to that shared state, and nuqs will **lift the state _out_** into the URL for you,
the same way you would use a global state manager like Zustand, Jotai etc.
...
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/U__Rwsp8v78?si=sQgg-HJTMoy_mt7K&amp;start=34" title="YouTube video player" frameBorder="0" allow="autoplay; encrypted-media; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen style={{ marginInline: 'auto'}}/>