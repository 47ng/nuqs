...
function MapLinks() {
  return (
    <>
      ...
      <Link
        href={
          getMapLink({ latitude: 51.51, longitude: -0.13 })
          // → /map?lat=51.51&lng=-0.13
        }
      >
        London
      </Link>
...
- It supports both Next.js & React Router typed routes <small>_(if we could connect to TSR too that'd be nice 👀)_</small>
- It handles static, dynamic & catch-all routes, with type-safe pathname params, search params, and hash.
...
<ul className="not-prose list-none space-y-1.5">
  <PullRequestLine number="996" /> <small className='text-gray-500'>(this helps with ESM/CJS interoperability)</small>
...
[GitHub](https://github.com/47ng/nuqs), [Bluesky](https://bsky.app/profile/nuqs.47ng.com) and [X/Twitter](https://x.com/nuqs47ng).
